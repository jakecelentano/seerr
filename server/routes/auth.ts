...
const getOidcRedirectUrl = (req: Request) => {
  ...
  // Prevent open redirects: the resolved URL is used as the OIDC redirect_uri,
  // so an absolute, attacker-controlled returnUrl must never override the
  // application origin. Fall back to the login page if origins differ.
...
authRoutes.post(
  ...
  async (
    ...
    try {
      ...
      logger.warn('Rejected OIDC callback with invalid callbackUrl', {
        label: 'Auth',
        provider: provider.slug,
        ip: req.ip,
      });
    ...
    const hasRequiredClaims = requiredClaims.every((claim) => {
      ...
      // Providers vary in how they serialize boolean claims: some emit a real
      // boolean, others a string (e.g. Authentik can return "true").
